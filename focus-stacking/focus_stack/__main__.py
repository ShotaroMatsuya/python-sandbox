...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run the focus stacking algorithm on a set of images."
    )

    parser.add_argument(
        "src_dir", metavar="SRC_DIR", type=str, help="the location of the source images"
    )
    parser.add_argument(
        "-d",
        "--dest-dir",
        ...
        help="the directory in which to place the stacked image",
    )
    parser.add_argument(
        "-c",
        "--choice",
        type=str,
        choices=[stk.CHOICE_PYRAMID, stk.CHOICE_AVERAGE, stk.CHOICE_MAX],
        default=stk.CHOICE_PYRAMID,
        dest="choice",
        help="the best-pixel selection strategy",
    )
    parser.add_argument(
        "-e",
        "--energy",
        type=str,
        choices=[stk.ENERGY_LAPLACIAN, stk.ENERGY_SOBEL],
        default=stk.ENERGY_LAPLACIAN,
        dest="energy",
        help="the energy function",
    )
    parser.add_argument(
        "-m",
        "--pyramid-min-size",
        type=int,
        default=32,
        dest="pyramid_min_size",
        help="the size of the smallest pyramid layer",
    )
    parser.add_argument(
        "-k",
        "--kernel",
        ...
        default=5,
        dest="kernel_size",
        help="the size of the kernel used in laplacian energy calculations",
    )
    parser.add_argument(
        "-b",
        "--blur",
        ...
        default=5,
        dest="blur_size",
        help="the size of the gaussian blur in laplacian energy calculation",
    )
    parser.add_argument(
        "-s",
        "--smooth",
        ...
        default=32,
        dest="smooth_size",
        help="the size of the kernel use in bilaterally filtering the energy matrix. Not used for pyramid.",
    )

    args = parser.parse_args()
    if args.choice == stk.CHOICE_PYRAMID and args.energy != stk.ENERGY_LAPLACIAN:
        raise RuntimeError(
            "The pyramid best-pixel selection strategy can only be used with laplacian energy"
        )
    if args.choice == stk.CHOICE_PYRAMID and args.kernel_size != 5:
        raise RuntimeError(
            "The pyramid best-pixel selection strategy can only be used with a kernel size of 5"
        )
